...
class qformat_htmltable extends qformat_xml {
    ...
    function presave_process( $content ) {
        ...
        if (!class_exists('XSLTProcessor') || !function_exists('xslt_create')) {
            ...
            notify(get_string('xsltunavailable', 'qformat_htmltable'));
            return false;
            ...
            notify(get_string('stylesheetunavailable', 'qformat_htmltable', $stylesheet));
            return false;
        }
        ...
        if (!strlen($content)) {
            ...
            notify(get_string('noquestions', 'qformat_htmltable'));
            return false;
        }
        ...
        if (!($temp_xml_filename = tempnam($CFG->dataroot . "/temp/", "ht1-"))) {
            ...
            notify(get_string('cannotopentempfile', 'qformat_htmltable', $temp_xml_filename));
            return false;
        }
        ...
        if (($nbytes = file_put_contents($temp_xml_filename, "<container>\n<quiz>" . $clean_content . "\n</quiz>\n" . $this->get_text_labels() . file_get_contents($iconfile_path) . "</container>")) == 0) {
            ...
            notify(get_string('cannotwritetotempfile', 'qformat_htmltable', $temp_xml_filename . "(" . $nbytes . ")"));
            return false;
        }
        ...
        if(!($xslt_output = xslt_process($xsltproc, $temp_xml_filename, $stylesheet, null, null, $parameters))) {
            ...
            notify(get_string('transformationfailed', 'qformat_htmltable', "(XSLT: " . $stylesheet . "; XML: " . $temp_xml_filename . ")"));
        ...
        if(!($xslt_output = xslt_process($xsltproc, $temp_xml_filename, $stylesheet, null, null, $parameters))) {
            ...
            notify(get_string('transformationfailed', 'qformat_htmltable', "(XSLT: " . $stylesheet . "; XHTML: " . $temp_xml_filename . ")"));
            $this->debug_unlink($temp_xml_filename);
            return false;
        }
