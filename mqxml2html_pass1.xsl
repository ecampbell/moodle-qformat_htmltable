...
<!-- Convert Moodle release numbers such as "2.4+" or "2.4.1" to "24" for easier numerical comparisons, to decide what question rows to include -->
<xsl:variable name="moodle_release_number" select="translate(substring($moodle_release, 1, 4), '.+', '')"/>
...
<!-- Base64-encoded image icons for each question type -->
<xsl:variable name="qtype_icons" select="/container/icons"/>
...
<xsl:variable name="correct_label" select="$moodle_labels/data[@name = 'quiz_correct']"/>
<xsl:variable name="incorrect_label" select="$moodle_labels/data[@name = 'quiz_incorrect']"/>
...
<xsl:variable name="shortanswer_correctanswers_label" select="$moodle_labels/data[@name = 'quiz_correctanswers']"/>
...
<xsl:template match="/container/quiz">
	<html>
		...
		<body>
			...
			<h1 class="MsoHeading1"><xsl:value-of select="$course_name"/></h1>
...
<xsl:template match="/container/icons"/>
<xsl:template match="/container/moodlelabels"/>

<!-- Category becomes a Heading 2 style -->
...
<xsl:template match="question[@type = 'category']">
	...
	<h2 class="MsoHeading2"><xsl:value-of select="concat($category_label, $colon_string, ' ', $category)"/></h2>
</xsl:template>
...
<xsl:template match="question">
	...
	<!-- Penalty value units: fractions in Moodle 1.9, percent in Moodle 2-->
	<xsl:variable name="penalty_units">
	...
	<xsl:variable name="penalty_value">
		<xsl:choose>
		<xsl:when test="$moodle_release_number = '19' and starts-with(penalty, '1')"><xsl:value-of select="concat('1', $penalty_units)"/></xsl:when>
		<xsl:when test="$moodle_release_number = '19' and starts-with(penalty, '0.5')"><xsl:value-of select="concat('0.5', $penalty_units)"/></xsl:when>
		<xsl:when test="$moodle_release_number = '19' and starts-with(penalty, '0.3333333')"><xsl:value-of select="concat('0.333', $penalty_units)"/></xsl:when>
		<xsl:when test="$moodle_release_number = '19' and starts-with(penalty, '0.25')"><xsl:value-of select="concat('0.25', $penalty_units)"/></xsl:when>
		<xsl:when test="$moodle_release_number = '19' and starts-with(penalty, '0.2')"><xsl:value-of select="concat('0.20', $penalty_units)"/></xsl:when>
		<xsl:when test="$moodle_release_number = '19' and starts-with(penalty, '0.1')"><xsl:value-of select="concat('0.10', $penalty_units)"/></xsl:when>
		<xsl:when test="starts-with(penalty, '1')"><xsl:value-of select="concat('100', $penalty_units)"/></xsl:when>
		<xsl:when test="starts-with(penalty, '0.5')"><xsl:value-of select="concat('50', $penalty_units)"/></xsl:when>
		<xsl:when test="starts-with(penalty, '0.3333333')"><xsl:value-of select="concat('33.3', $penalty_units)"/></xsl:when>
		<xsl:when test="starts-with(penalty, '0.25')"><xsl:value-of select="concat('25', $penalty_units)"/></xsl:when>
		<xsl:when test="starts-with(penalty, '0.2')"><xsl:value-of select="concat('20', $penalty_units)"/></xsl:when>
		<xsl:when test="starts-with(penalty, '0.1')"><xsl:value-of select="concat('10', $penalty_units)"/></xsl:when>
	...
	<xsl:variable name="colheading1_label">
		<xsl:choose>
		<xsl:when test="$qtype = 'MA' or $qtype = 'MC' or $qtype = 'MS'"><xsl:text>#</xsl:text></xsl:when>
		<xsl:otherwise><xsl:value-of select="$blank_cell"/></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="colheading2_label">
		...
		<xsl:when test="$qtype = 'MA' or $qtype = 'MC' or $qtype = 'MS'"><xsl:value-of select="$choice_label"/></xsl:when>
		<xsl:when test="$qtype = 'MAT'"><xsl:value-of select="$question_label"/></xsl:when>
		<xsl:when test="$qtype = 'SA'"><xsl:value-of select="$answers_label"/></xsl:when>
	...
	<xsl:variable name="colheading3_label">
		...
		<xsl:when test="$qtype = 'MA' or $qtype = 'MC' or $qtype = 'MS' or $qtype = 'SA' or $qtype = 'TF'"><xsl:value-of select="$feedback_label"/></xsl:when>
		<xsl:otherwise><xsl:value-of select="$blank_cell"/></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="colheading4_label">
		<xsl:choose>
		<xsl:when test="$qtype = 'MA' or $qtype = 'MC' or $qtype = 'MS' or $qtype = 'SA' or $qtype = 'TF'">
	...
	<xsl:variable name="question_type">
		<xsl:choose>
		<xsl:when test="@type = 'cloze'">
			<xsl:value-of select="'multianswer'"/>
		</xsl:when>
	...
	<xsl:variable name="question_label">
		<xsl:if test="$moodle_release_number &gt; '19'">
			<xsl:value-of select="concat(' (', $moodle_labels/data[@name = concat('qtype_', $question_type, '_pluginname')], ')')"/>
		</xsl:if>
	</xsl:variable>

	<h3 class="MsoHeading3">
		<img src="{$qtype_icons/data[@name = $question_type]}" width="16" height="16" alt="{$question_label}"/>
		<xsl:value-of select="concat(' ', normalize-space(name), $question_label)"/>
	</h3>
	...
	<thead>
		...
		<xsl:if test="$qtype != 'CL' and $qtype != 'DE'">
			<tr>
		...
		<xsl:if test="$qtype = 'CL' or $qtype = 'MA' or $qtype = 'MAT' or $qtype = 'MC' or $qtype = 'MS' or $qtype = 'SA'">
			<tr>
		...
		<!-- Show number of correct responses when finished (Moodle 2.x only). omit if No -->
		<xsl:if test="shownumcorrect and ($qtype = 'MA' or $qtype = 'MAT' or $qtype = 'MS') and $moodle_release_number &gt; '19'">
			<tr>
				<td colspan="3" style="{$col2_2span_width}"><p class="TableRowHead" style="text-align: right"><xsl:value-of select="$shownumcorrectfeedback_label"/></p></td>
				<td style="width: 1.0cm"><p class="Cell"><xsl:value-of select="$yes_label"/></p></td>
			</tr>
			<xsl:text>&#x0a;</xsl:text>
		</xsl:if>
		...
		<tr>
			<xsl:choose>
			<xsl:when test="$qtype = 'ES' or $qtype = 'MA' or $qtype = 'MC' or $qtype = 'MS'">
	...
	<xsl:text>&#x0a;</xsl:text>
		...
		<xsl:if test="$qtype = 'MA' or $qtype = 'MAT' or $qtype = 'MC' or $qtype = 'MS'">
			<xsl:if test="normalize-space(correctfeedback/text) != '' and normalize-space(correctfeedback/text) != '&#160;'">
				<tr>
					<th colspan="2" style="{$col2_2span_width}"><p class="TableRowHead"><xsl:value-of select="$correctfeedback_label"/></p></th>
					<td colspan="2" style="{$col3_2span_width}"><xsl:apply-templates select="correctfeedback/*"/></td>
				</tr>
				<xsl:text>&#x0a;</xsl:text>
			</xsl:if>
			<xsl:if test="normalize-space(incorrectfeedback/text) != '' and normalize-space(incorrectfeedback/text) != '&#160;'">
				<tr>
					<th colspan="2" style="{$col2_2span_width}"><p class="TableRowHead"><xsl:value-of select="$incorrectfeedback_label"/></p></th>
					<td colspan="2" style="{$col3_2span_width}"><xsl:apply-templates select="incorrectfeedback/*"/></td>
				</tr>
				<xsl:text>&#x0a;</xsl:text>
			</xsl:if>
		</xsl:if>
		<!-- Partially correct feedback for MA (Multi-answer) and MAT questions only -->
		<xsl:if test="$qtype = 'MA' or $qtype = 'MAT'">
			<xsl:if test="normalize-space(partiallycorrectfeedback/text) != '' and normalize-space(partiallycorrectfeedback/text) != '&#160;'">
				<tr>
					<th colspan="2" style="{$col2_2span_width}"><p class="TableRowHead"><xsl:value-of select="$pcorrectfeedback_label"/></p></th>
					<td colspan="2" style="{$col3_2span_width}"><xsl:apply-templates select="partiallycorrectfeedback/*"/></td>
				</tr>
				<xsl:text>&#x0a;</xsl:text>
			</xsl:if>
		</xsl:if>
		...
		<xsl:if test="$moodle_release_number &gt; '19'">
			<xsl:for-each select="hint[normalize-space(text) != '' and normalize-space(text) != '&#160;']">
				...
				<tr>
					<th colspan="2" style="{$col2_2span_width}"><p class="TableRowHead"><xsl:value-of select="concat($hint_number_label, $colon_string)"/></p></th>
					<td colspan="2" style="{$col3_2span_width}">
						<xsl:apply-templates/>
					</td>
				</tr>
				<xsl:text>&#x0a;</xsl:text>
				<!-- Most question types allow for some fields on the behaviour of hints, but SA doesn't, and CL only in 2.4+ -->
				<xsl:if test="($qtype = 'MA' or $qtype = 'MAT' or $qtype = 'MC' or $qtype = 'MS') or ($qtype = 'CL' and $moodle_release_number &gt; '23')">
					<tr>
						<th colspan="2" style="{$col2_2span_width}"><p class="TableRowHead"><xsl:value-of select="concat($hint_clearwrongparts_label, ' (', $hint_number_label, ')', $colon_string)"/></p></th>
						<td colspan="2" style="{$col3_2span_width}"><p class="Cell">
							<xsl:choose>
			...
			<xsl:if test="tags != '' and tags != '&#160;'">
				<tr>
					<th colspan="2" style="{$col2_2span_width}"><p class="TableRowHead"><xsl:value-of select="$tags_label"/></p></th>
					<td colspan="2" style="{$col3_2span_width}">
						<p class="Cell">
								<xsl:choose>
...
<xsl:template match="answer[ancestor::question/@type = 'truefalse']" priority="2">
	<tr>
		<td colspan="2" style="{$col2_2span_width}">
			<xsl:choose>
		...
		<td style="width: 1.0cm"><p class="QFGrade">
			<xsl:choose>
			<xsl:when test="starts-with(@fraction, '0')">
				<img src="{$qtype_icons/data[@name = 'incorrect']}" alt="{$incorrect_label}"/>
			</xsl:when>
			<xsl:otherwise><img src="{$qtype_icons/data[@name = 'correct']}" alt="{$correct_label}"/></xsl:otherwise>
			</xsl:choose>
		</p></td>
	</tr>
</xsl:template>
...
<xsl:template match="answer[not(ancestor::subquestion)]|subquestion">
	...
	<xsl:variable name="numbercolumn_class">
		<xsl:choose>
		<xsl:when test="$qtype = 'MAT' or $qtype = 'SA'">
	...
	<tr>
		...
		<xsl:when test="$qtype = 'MAT'">
			<td colspan="2" style="{$col2_2span_width}"><p class="Cell"><xsl:apply-templates select="text|file"/></p></td>
		</xsl:when>
		<xsl:when test="$qtype = 'SA'">
			<td colspan="2" style="{$col2_2span_width}"><p class="Cell"><xsl:value-of select="normalize-space(text)"/></p></td>
		</xsl:when>
		<xsl:otherwise>
			<td style="width: 1.0cm"><ol><li type="{$numbering_flag}" value="{position()}"></li></ol></td>
		...
		<xsl:choose>
		<xsl:when test="$qtype = 'MAT'">
			<td colspan="2" style="{$col3_2span_width}"><p class="Cell"><xsl:value-of select="answer/text"/></p></td>
		</xsl:when>
		<xsl:otherwise>
			<td style="{$col3_width}"><p class="QFFeedback"><xsl:apply-templates select="feedback/text|feedback/file"/></p></td>
			<td style="width: 1.0cm"><p class="QFGrade">
				<!-- Include the correct/incorrect icon-->
				<xsl:choose>
				<xsl:when test="starts-with(@fraction, '0') or starts-with(@fraction, '-')">
					<img src="{$qtype_icons/data[@name = 'incorrect']}" alt="{$incorrect_label}"/>
				</xsl:when>
				<xsl:otherwise><img src="{$qtype_icons/data[@name = 'correct']}" alt="{$correct_label}"/></xsl:otherwise>
				</xsl:choose>
				<!-- Include the numeric grades for MA and SA questions -->
				<xsl:if test="not(starts-with(@fraction, '0')) and ($qtype = 'MA' or $qtype = 'SA')">
					<xsl:value-of select="concat('&#160;(', $fraction_value, ')')"/>
				</xsl:if>
			</p></td>
		</xsl:otherwise>
		</xsl:choose>
